...
#获取某个文件夹下某种后缀的文件列表，包括子文件夹里面的
def get_file_list(self_dir,suffix):
    all_file = []
    #路径下所有非目录子文件
...
def convert(self_is_related,self_child_file_location,self_total_sheet,self_q):
    try:
        print("converting:"+self_child_file_location)
        line_num_in_child = 13 if(self_is_related)  else 14
        ...
        #需要修改汇总表的哪一行
        key_name = get_key_name(self_child_file_location)
        line_num_in_total = get_line_number(self_total_sheet,key_name,self_is_related)
        result = get_need_data_in_child(child_sheet,line_num_in_child)
        write_need_data_to_total(self_total_sheet,line_num_in_total,result)

        # Save the file
        child_book.save(self_child_file_location)
        #注意如果原文件有一些图片或者图标，则保存的时候可能会导致图片丢失

        mes = "#转换成功#:"+self_child_file_location
        self_q.put(mes)

    except Exception as e:
        print(e)
        mes = "#转换失败#:"+self_child_file_location+"出现了异常:"+str(e)
        self_q.put(mes)

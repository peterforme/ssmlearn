...
#是否为关联
is_related = False
#关联则是在子表的第13行，非关联则是在第14行,该部分目前是写死的
line_num_in_child = 13 if(is_related)  else 14

child_file_location = r'C:\excel\2018年7月-应收-其他应收-预付款项汇总表-物流.xlsx'
child_book = load_workbook(child_file_location, data_only=True)
child_book.guess_types = True   #猜测格式类型
child_sheet=child_book.active

total_file_location = r'C:\excel\汇总表.xlsx'
...
#需要修改汇总表的哪一行
key_name = get_key_name(child_file_location)
line_num_in_total = get_line_number(total_sheet,key_name,is_related)
result = get_need_data_in_child(child_sheet,line_num_in_child)
write_need_data_to_total(total_sheet,line_num_in_total,result)


# Save the file
child_book.save(child_file_location)
